...
const convertTextToAudio = async (rawText, pathOfOutputFile) => {
  ...
  const mergedpath = `${__dirname}/outputAudio`;
  ...
  // delete all the files from the `/audio` folder
  fs.readdirSync(mergedpath).forEach((f) => fs.rmSync(`${mergedpath}/${f}`));
  ...
  await mergeAudioFiles(totalfiles, pathOfOutputFile, timing);
};
